module.exports = function(print, debug){
	...
	Memory.prototype.write = function(addr, value32){
		...
		if(!this.memory_segments[seg]) this.memory_segments[seg] = new Uint32Array(this.SEGMENT_SIZE);
		this.memory_segments[seg][addr - seg*this.SEGMENT_SIZE] = value32;
		//console.log('WRITE '+value32+' AT '+addr+' (SEG '+seg+')');
	}

	Memory.prototype.read = function(addr){
		var seg = (addr / this.SEGMENT_SIZE) >> 0;
		if(!this.memory_segments[seg]) return 0;
		return this.memory_segments[seg][addr - seg*this.SEGMENT_SIZE];
	}
